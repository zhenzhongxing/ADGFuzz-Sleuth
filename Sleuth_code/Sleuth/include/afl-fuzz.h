...
struct queue_entry {
  ...
  /* Add by wei as 16/5/2023 */
  u8 *trace_target;                      // replce the target_bits to mini
  u8 *target_mini;                       // Target trace bytes, if kept
  bool has_new_target;                   // Triggers new target?
  u32 target_ref;                        // target bytes ref count
  u16 min_level;                         // minimum level in a paht
  /* Add End */
...
typedef struct afl_state {
  ...
  u32 *related_bits,
     *related_crash;                        /* Regions of target untouched by fuzzing, like virgin_bits */
  u32 *related_count;                      /*  */

  u8 *virgin_level;                   /* edge of MRG yet untouched by fuzzing*/

  u8 *high_level;                    /* get the highest level a seed through */

  u32 virgin_capability;               /* Capability hash of original poc */
  u32 virgin_position;                 /* Position hash of original poc */
...
#define N_FUZZ_SIZE (1 << 21)
  ...
  u32 queued_paths,                     /* Total number of queued testcases */
      ...
      queued_with_target,               /* Add by wei -- Paths with new target*/
  ...
  struct queue_entry **target_top;          /* Add by wei --Top entries for target bitmap bytes */
  ...
  u32 *reach_entry_count,
      *trigger_entry_count;

  u32 current_targets_triggered,
      current_targets_reached;

  u32 single_exec_triggered,    // the target's number in one fuzz exec 
      single_exec_reached;
  ...
  u8 start_depth_exploration,
     start_breadth_exploration,
     new_cap,
     new_pos;
  ...
  u8 *target_debug_log,
     *queue_debug_log,
     *level_debug_log,
     *basicblock_debug_log,
     *pos_debug_log,
     *switch_debug_log,
     *count_debug_log;

  FILE *target_debug_fd,
       *queue_debug_fd,
       *level_debug_fd,
       *basicblock_debug_fd,
       *pos_debug_fd,
