...
inline void classify_target_counts(afl_forkserver_t *fsrv) {

  u64 *mem = (u64 *)fsrv->target_bits;
 ...
 * seen in ret. */
inline void discover_entry(u8 *ret, u64 *current, u64 *virgin) {
  ...
  if (*current & *virgin) {
    ...
    if (likely(*ret < 2)) {
      ...
      if ((cur[0] && vir[0] == 0xffffffff) || (cur[1] && vir[1] == 0xffffffff)){
        *ret = 2;
      }
      else{
        *ret = 1;
