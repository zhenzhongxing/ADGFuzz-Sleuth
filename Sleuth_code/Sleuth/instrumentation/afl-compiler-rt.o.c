...
/* Add by wei at 9/5/2023 */
static u32 __target_bb_initial[TARGET_MAP];
u32 *__target_bb_ptr = __target_bb_initial;
...
/* Add by wei*/
u32 __afl_target_size = TARGET_MAP;
/* Add end */
...
static void __afl_map_shm(void) {
  ...
  /* Add by wei at 9/5/2023 */
  char *target_str = getenv(TARGET_BB);

  if (__afl_debug) {

    fprintf(stderr, "DEBUG: target target_str %s\n", target_str == NULL ? "<null>" : target_str);
  ...
  if (target_str) {
    ...
    u32 shm_id = atoi(target_str);
    __target_bb_ptr = shmat(shm_id, NULL, 0);

    if (!__target_bb_ptr || __target_bb_ptr == (void *)-1){

      perror("shmat for targetlog");
      send_forkserver_error(FS_ERROR_SHM_OPEN);
      _exit(1);
...
static void __afl_unmap_shm(void) {
  ...
  id_str = getenv(TARGET_BB);
  if (id_str){
#ifdef USEMMAP
    
    munmap((void *)__target_bb_ptr, __afl_target_size);

#else

    shmdt((void *)__target_bb_ptr);

#endif

    __target_bb_ptr = NULL;
