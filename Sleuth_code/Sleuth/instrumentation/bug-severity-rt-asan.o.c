...
static u8 *__afl_sleuth_res_path;
...
static inline void __afl_evo_TryInit() {
    if (likely(!__afl_evo_already_init)) {
        ...
        if (!(__afl_sleuth_res_path = getenv(SLEUTH_ENV_RESPATH)))
            { __afl_sleuth_res_path = SLEUTH_DEFAULT_RESPATH; }
...
static inline void __afl_sleuth_SaveCap(char *pos_text, u8 hashed) {
    if (unlikely(!__afl_sleuth_res_path)) return;
    FILE *fp = fopen(__afl_sleuth_res_path, "w");
    if (likely(fp)) {
        if (unlikely(!hashed)) {
            fprintf(fp, "%s", pos_text);
            //fprintf(stderr, "position HASH: %u", pos_test);
        } else {
            u32 sleuth_hash = __afl_evo_GetHash(pos_text, strlen(pos_text));
            fwrite(&sleuth_hash, sizeof(u32), 1, fp);
            //fprintf(stderr, "POS HASH: %u", sleuth_hash);
        }
        fclose(fp);
    } else {
        fprintf(stderr, "%s: Unable to create '%s'", __func__, __afl_sleuth_res_path);
    }
}
...
static inline u32 __afl_sleuth_ChkStackHash() {
    u32 pos_hash = 0;
    void* stack_frames[SLEUTH_STACK_FRAME_MAXNUM];
    int num_frames = backtrace(stack_frames, SLEUTH_STACK_FRAME_MAXNUM);
    if (unlikely(num_frames <= SLEUTH_SKIP_FRAME))
    {
        
        fprintf(stderr, "%s: trace failed", __func__);
        return pos_hash;
        
    }
    
    if (num_frames > SLEUTH_SKIP_FRAME){
        u32 pos_len = 0;
        char *pos = (char *) calloc(1 + (num_frames - SLEUTH_SKIP_FRAME) * (2 + (sizeof(void*) << 1)), sizeof(char));
        if (unlikely(!pos)){
            fprintf(stderr, "%s: calloc failed", __func__);
            return pos_hash;
        }
        size_t write_pos = 1 + 2 + (sizeof(void*) << 1);
        int write_some = 0;
        for (int i = SLEUTH_SKIP_FRAME; i < num_frames; ++i) {
            write_some = snprintf((pos + pos_len), write_pos, "%p", stack_frames[i]);
            if (write_some > 10) continue;
            if (unlikely(write_some < 0 || write_some >= write_pos)) break;
            pos_len += write_some;
        }
        pos_hash = __afl_evo_GetHash(pos, pos_len);
        free(pos);
    }
    //free(stack_frames);
    return pos_hash;
...
static inline void __afl_evo_GetCapability() {
    ...
    char * pos_test_buf = NULL; //position text
    ...
    u64 posStackHash = __afl_sleuth_ChkStackHash(); 
    char posStackHash_buf[1 + 20];
    snprintf(posStackHash_buf, sizeof(posStackHash_buf), "%llu", posStackHash);
    ...
    if (unlikely(!__afl_evo_using_capfuzz)) {
        ...
        pos_test_buf = (char *) calloc((
                strlen(bugT)             + sizeof(EVOCATIO_IDENTIFIER)-1 + \
                strlen(operaT)           + sizeof(EVOCATIO_IDENTIFIER)-1 + \
                strlen(posStackHash_buf) + sizeof(EVOCATIO_IDENTIFIER)
        ), sizeof(char));       //sleuth special

        if(unlikely(!cap_text_buf || !pos_test_buf)) {
            fprintf(stderr, "%s: calloc failed", __func__);
            return;
        }
        ...
        //strcat(pos_test_buf, allocStackHash_buf); strcat(pos_test_buf, EVOCATIO_IDENTIFIER);    //sleuth special
        strcat(pos_test_buf, bugT);             strcat(pos_test_buf, EVOCATIO_IDENTIFIER);
        strcat(pos_test_buf, operaT);           strcat(pos_test_buf, EVOCATIO_IDENTIFIER);
        strcat(pos_test_buf, posStackHash_buf); strcat(pos_test_buf, EVOCATIO_IDENTIFIER);
        

        __afl_evo_SaveCap(cap_text_buf, 0);
        __afl_sleuth_SaveCap(pos_test_buf, 0);
        free(cap_text_buf);
        free(pos_test_buf);
        return;
    }
   ...
   /*
    cap_text_buf = (char *) calloc((
                strlen(bugT)            + strlen(operaT) + \
                strlen(invalidAddr_buf) + strlen(accessLen_buf) + 1
            ), sizeof(char));
    */
    ...
    //strcat(pos_test_buf, allocStackHash_buf);
    strcat(pos_test_buf, bugT);
    strcat(pos_test_buf, operaT);
    strcat(pos_test_buf, posStackHash_buf);

    __afl_evo_SaveCap(cap_text_buf, 1);
    __afl_sleuth_SaveCap(pos_test_buf, 1);
    free(cap_text_buf);
    free(pos_test_buf);
    return;
}
