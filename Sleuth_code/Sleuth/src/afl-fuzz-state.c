...
extern khash_t(m64HashTable) *heatMapUnion;
extern khash_t(m64HashTable) *all_cap_hashTable;
extern khash_t(m64HashTable) *all_pos_hashTable;
...
void afl_state_init(afl_state_t *afl, uint32_t map_size) {
  ...
  afl->related_bits = ck_alloc(sizeof(u32) * TARGET_MAP);
  afl->related_crash = ck_alloc(sizeof(u32) * TARGET_MAP);
  afl->target_top = ck_alloc(TARGET_MAP * sizeof(void *));
  /*--Add End--*/
...
void capability_state_destroy(afl_state_t *afl) {
  /* Destroy hash table */
  kh_destroy(m64HashTable, heatMapUnion);
  kh_destroy(m64HashTable, all_cap_hashTable);
  kh_destroy(m64HashTable, all_pos_hashTable);

  /* Free capability malloc memory */
  //ck_free(d_related_pos);
  ck_free(all_union);
  ck_free(original_poc_buf);
...
void afl_state_deinit(afl_state_t *afl) {
  ...
  /* Add by wei at 29/5/2023 */
  ck_free(afl->related_bits);
  ck_free(afl->related_crash);
  ck_free(afl->target_top);
  /* Add End */
