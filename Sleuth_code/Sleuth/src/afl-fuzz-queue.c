...
//创建可实现加权随机选取的别名表，运算开销较大

void create_alias_table(afl_state_t *afl) { //该函数的作用是创建一个别名表，以实现加权随机选择。别名表是一种数据结构，可以在常数时间内根据权重随机选择元素。该函数首先计算每个队列条目的权重，然后使用这些权重来构建别名表和概率表，以便在后续的随机选择中使用。
...
/* Add by wei at 22/5/2023 : compress two bit bitmap to one bit map*/
void compress_target(u8* dst, u8* src) {
  for (u32 i = 0; i < MAP_SIZE; i++){

    if (src[i >> 2] & (1 << ((i & 3) * 2 + 1))) {
...
/*
void update_level_cov(afl_state_t *afl, u32* target_map){

  if (unlikely(!afl->virgin_level)) {

    afl->virgin_level = ck_alloc(sizeof(char) * TARGET_MAP);

  }

  for (u32 i = 0; i < TARGET_MAP; i++){
    if (target_map[i] && !afl->virgin_level[i]){
      afl->virgin_level[i] = 1;
    }
  }
...
void add_to_queue(afl_state_t *afl, u8 *fname, u32 len, u8 passed_det) {
  ...
  /*
  if (afl->shm.targetfuzz_mode && afl->shm.target_map){
    update_level_cov(afl, afl->shm.target_map);
  }
  */
...
/* Add by wei at 17/5/2023 */

void update_level_target(afl_state_t *afl, struct queue_entry *q){

  u16 address;
  u32 i;
  u32 len;
  u64 fav_factor = q->exec_us * q->len;
  u16 max_level = LOWESTLEVEL;
  u16 current_level;
  u16 level_num = 0;
  u32 bb_idx_level[TARGET_MAP];
  u16 bb_idx[TARGET_MAP];
  u16 bb_level[TARGET_MAP];

  // Step 1 : We must ensure that we pass through the target, if not the queue will not change
  //if (!afl->fsrv.target_bits) return;

  // Initial the level list

  // Step 2 : We choose the seed with highest level, if the level is same, we compare the seed length and exec time
  // Now we first find the minimum level
  for (i = 0; i < TARGET_MAP; ++i){

    if (afl->fsrv.target_bits[i]){

      current_level = afl->fsrv.target_bits[i] & 0xFFFF;
      ...
      bb_idx_level[level_num] = afl->fsrv.target_bits[i];
      bb_idx[level_num] = (afl->fsrv.target_bits[i] >> 16) & 0xFFFF;
      bb_level[level_num] = (afl->fsrv.target_bits[i]) & 0xFFFF;
      level_num ++;
       
    }
  }
  q->min_level = max_level;

  //write_union(afl, fuzzed_union);

  //write_level_log(afl, max_level, level_num);

  //write_bb_log(afl, bb_idx_level, level_num);

  for (i = 0; i < TARGET_MAP; ++i){

    if (afl->fsrv.target_bits[i]){

      address = afl->fsrv.target_bits[i] & 0xFFFF;
      
      //write_check_log(afl, level_num, address, TARGET_MAP);

      if (afl->target_top[i]){

        if (address > afl->target_top[i]->min_level){
          continue;
        }
        else if (address == afl->target_top[i]->min_level && fav_factor > afl->target_top[i]->exec_us * afl->target_top[i]->len){
          continue;
        }
      }
...
void update_bitmap_score_target(afl_state_t *afl, struct queue_entry *q) {
  ...
  for (i = 0; i < afl->fsrv.map_size; ++i){
    ...
    // Step 1 : We must ensure that we pass through the target, if not, the flag will not change.
    if (afl->fsrv.target_bits[i]){

      address = afl->fsrv.target_bits[i];

      len = afl->fsrv.map_size;

      write_check_log(afl, i, address, len);

      // Step 2 : If there are new target entries, we make these seeds as favorite.

      if (afl->target_top[i]){

        if (fav_factor > afl->target_top[i]->exec_us * afl->target_top[i]->len){
          continue;
        }

        if (!--afl->target_top[i]->target_ref){

          ck_free(afl->target_top[i]->target_mini);
          afl->target_top[i]->target_mini = 0;
      ...
      if (!q->target_mini) {
        ...
        q->target_mini = ck_alloc(len);
        minimize_bits(afl, q->target_mini, afl->fsrv.target_bits);
...
/* Add by wei at 16/5/2023 */
// Here we add new mechanism to cull_queue, and we define our favorite fuzzing step
// First! We take the new entry in target bitmap as the top queue;
// Second! If the entry in target bitmap is old, we take the new entry in edge bitmap as the top queue;  

void target_cull_queue(afl_state_t *afl){


  /* Step 1: Though the target */

  if (likely(!afl->target_changed || afl->non_instrumented_mode)) {
    return;
  }
  ...
  for (u32 i = 0; i < afl->queued_paths; i++){
    ...
    afl->queue_buf[i]->favored = 0;           // We init the queue buffer
  
  }  

  /* Step 2 : Found the new entry */

  for (u32 i = 0; i < TARGET_MAP; ++i) {

    if (afl->target_top[i] && !afl->related_bits[i]) {  // If the target has a testcase, and is a new testcase 
    //if (afl->target_top[i] && !afl->virgin_level[i]){

      if (afl->target_top[i]->favored) continue;

      afl->target_top[i]->favored = 1;
      afl->queued_favored++;

      if (!afl->target_top[i]->was_fuzzed){
...
void pos_cull_queue(afl_state_t *afl) {
  ...
  /* We take the seed that discover new position as favorite. */
  u32 fuzzed_pos_cnt_ori = fuzzed_pos_cnt;
  struct queue_entry *q;
  int is_fuzzed = 0;
  ...
  for (u32 i = 0; i < afl->queued_paths; i++){
    ...
    //if (!is_fuzzed && q->position_hash) {
    if (!is_fuzzed) {

      q->favored = 1;
      afl->queued_favored++;
      fuzzed_pos[fuzzed_pos_cnt] = q->position_hash;
      fuzzed_pos_cnt += 1;

      if (!q->was_fuzzed) afl->pending_not_fuzzed++;

    } 
    //else if (fuzzed_switch_pos_cnt < fuzzed_pos_cnt_ori && q->position_hash){
    else if (fuzzed_switch_pos_cnt < fuzzed_pos_cnt_ori){

      for (u32 m = fuzzed_switch_pos_cnt; m < fuzzed_pos_cnt_ori; m++){
        if (q->position_hash == fuzzed_pos[m]){
          q->favored = 1;
          afl->queued_favored++;
      ...
      //fuzzed_switch_pos_cnt = fuzzed_pos_cnt_ori;
    }
...
/*
  ...
  do selection among two stages

*/

void cull_queue(afl_state_t *afl) {

  if (get_cur_time() - afl->last_log_time > LOG_TIME){

    write_log(afl, afl->new_cap, afl->new_pos, afl->start_depth_exploration, all_switch_num, all_position_num);
    //write_pos(afl, fuzzed_pos, fuzzed_switch_pos_cnt);
    //write_pos(afl, fuzzed_pos);

  }

  u8 switch_flag;
  u8 old_exploration = afl->start_depth_exploration;

  /*INITIAL SWITCH TIME*/
  if (!afl->last_check_exec || !afl->last_check_time){
  ...
  if (!afl->start_depth_exploration){
	...
	if (get_cur_time() - afl->last_check_time >= MAXIUM_SWITCH_TIME){
      ...
      /*Check whether get the max exec*/
      if (afl->fsrv.total_execs - afl->last_check_exec >= MINIMAL_SWITCH_EXEC){
        
        afl->last_check_exec = afl->fsrv.total_execs;
        if (!afl->new_cap) {
          switch_flag = 0;
          fuzzed_switch_pos_cnt = fuzzed_pos_cnt;
        }
        afl->new_cap = 0;

      }
      /*Check whether get the max time*/
      if (get_cur_time() - afl->last_check_time >= MAXIUM_SWITCH_TIME){
  ...
  afl->start_depth_exploration = switch_flag;

  if (afl->start_depth_exploration != old_exploration){
    all_switch_num += 1;
  }

  if (!afl->start_depth_exploration){
    target_cull_queue(afl);
  }else{
    pos_cull_queue(afl);
  }
