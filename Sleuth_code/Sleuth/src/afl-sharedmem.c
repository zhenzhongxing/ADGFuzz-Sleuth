...
void afl_shm_deinit(sharedmem_t *shm) {
  ...
  if (shm->targetfuzz_mode) { shmctl(shm->target_shm_id, IPC_RMID, NULL); }
#endif
...
u8 *afl_shm_init(sharedmem_t *shm, size_t map_size,
  ...
  shm->target_map = NULL;       /* Add by wei at 17/5/2023 */
  ...
  if (shm->targetfuzz_mode) {

    shm->target_shm_id = shmget(IPC_PRIVATE, sizeof(u32) * TARGET_MAP, IPC_CREAT | IPC_EXCL | DEFAULT_PERMISSION);
    if (shm->target_shm_id < 0) {
  ...
  /* Add by wei at 17/5/2023 */
  if (shm->targetfuzz_mode && ! non_instrumented_mode) {

    shm_str_target = alloc_printf("%d", shm->target_shm_id);
    
    setenv("__TARGET_BB_ID", shm_str_target, 1);
    
    ck_free(shm_str_target);
  ...
  if (shm->map == (void *)-1 || !shm->map) {
    ...
    if (shm->targetfuzz_mode){
      ...
      shmctl(shm->target_shm_id, IPC_RMID, NULL);
  ...
  if (shm->targetfuzz_mode) {
    ...
    shm->target_map = shmat(shm->target_shm_id, NULL, 0);

    if (shm->target_map == (void *)-1 || !shm->target_map) {
