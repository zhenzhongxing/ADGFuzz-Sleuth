...
fuzz_run_target(afl_state_t *afl, afl_forkserver_t *fsrv, u32 timeout) {
  ...
  /* Add by wei at 18/5/2023 */

  if (afl->shm
          .target_map) {  // 如果开启了目标导向模式，执行前先清空目标覆盖率地图
    
    memset(afl->shm.target_map, 0x00000000, TARGET_MAP);

  }
  /* --Add End */

  fsrv_run_result_t res = afl_fsrv_run_target(
      fsrv, timeout,
      &afl->stop_soon);  // 作用：执行目标程序，即在 fuzzing
                         // 过程中执行目标程序，并监控其执行状态和时间，以便在
                         // fuzzing 过程中能够根据目标程序的执行状态和时间控制
                         // fuzzing 的行为和效率，促进 fuzzing 的效果提升

#ifdef PROFILING  // 作用：记录工作时间，即在 fuzzing 过程中记录工作时间，以便在 fuzzing
                         // 过程中能够根据工作时间控制 fuzzing 的行为和效率，促进 fuzzing 的效果提升
  clock_gettime(CLOCK_REALTIME, &spec);
...
void __attribute__((hot)) //__attribute__((hot)) 是 GCC 的一个函数属性，用于告诉编译器这个函数是一个热点函数，即在程序执行过程中被频繁调用的函数。编译器可以根据这个信息进行优化，比如将这个函数放在更快的内存区域，或者使用更高效的调用约定，以提高程序的性能。在 afl-fuzz 中，fuzz_run_target 和 write_to_testcase 这两个函数被标记为热点函数，因为它们在 fuzzing 过程中被频繁调用，对性能有较大影响。
           write_to_testcase(
               afl_state_t *afl, void *mem,
               u32 len) {  // 作用：写入测试用例，即在 fuzzing
                           // 过程中将修改后的测试用例写入文件或共享内存，以便在
                           // fuzzing 过程中能够根据修改后的测试用例控制 fuzzing
                           // 的行为和效率，促进 fuzzing 的效果提升

#ifdef _AFL_DOCUMENT_MUTATIONS  // 作用：记录变异测试用例，即在 fuzzing
                          // 过程中记录变异测试用例，以便在 fuzzing
                           // 过程中能够根据记录的变异测试用例分析 fuzzing
                           // 的行为和效率，促进 fuzzing 的效果提升
  s32  doc_fd;
  char fn[PATH_MAX];
...
u8 calibrate_case(afl_state_t *afl, struct queue_entry *q, u8 *use_mem,
  ...
  /* Add by wei at 18/5/2023 */
  if (afl->shm.targetfuzz_mode) {

    if (!q->trace_target) {

      q->trace_target = ck_alloc(sizeof(u8) * MAP_SIZE / 8);
      //compress_target(q->trace_target, afl->shm.target_map);

    }

  }

  q->min_level = LOWESTLEVEL;
  /* --Add End */
...
/* Add by wei at 22/5/2023 : update target_fuzz states */
void update_edge_states(afl_state_t *afl, u8* bitmap){

  if (unlikely(!afl->reach_bits_count)) {

    afl->reach_bits_count = ck_alloc(sizeof(u32) * MAP_SIZE);

  }

  u8 *map_zion = bitmap;
  //afl->single_exec_reached = 0;

  for (u32 i = 0; i < MAP_SIZE; i++){

    if (unlikely(map_zion[i])) {

      if (!afl->reach_bits_count[i]){

        afl->current_targets_reached++;
        //afl->single_exec_reached++;
...
void update_target_states(afl_state_t *afl, u32* target_map){

  if (unlikely(!afl->trigger_bits_count)) {

    afl->trigger_bits_count = ck_alloc(sizeof(u32) * TARGET_MAP);

  }

  u32 *map_zion = target_map;
  //afl->single_exec_triggered = 0;
...
//编写修改后的测试用例，运行程序并处理结果。处理异常情况，需终止程序时返回数值 1。该函数是模糊测试主函数的辅助函数。
...
common_fuzz_stuff(afl_state_t *afl, u8 *out_buf, u32 len) { 
  ...
  write_to_testcase(
      afl, out_buf,
      len);  // 作用：写入测试用例，即在 fuzzing
             //  过程中将修改后的测试用例写入文件或共享内存，以便在
             //  fuzzing 过程中能够根据修改后的测试用例控制 fuzzing
             //  的行为和效率，促进 fuzzing 的效果提升

  fault = fuzz_run_target(afl, &afl->fsrv, afl->fsrv.exec_tmout); // 作用：执行目标程序，即在 fuzzing
                                                        // 过程中执行目标程序，并监控其执行状态和时间，以便在
                                                        // fuzzing 过程中能够根据目标程序的执行状态和时间控制
                                                        // fuzzing 的行为和效率，促进 fuzzing 的效果提升

  if (afl->shm
          .targetfuzz_mode) {  // 如果开启了目标导向模式，执行后更新目标覆盖率地图和边覆盖率地图
    
    update_target_states(afl, afl->shm.target_map);
    update_edge_states(afl, afl->shm.map);
    //write_target_log(afl);

  }
  
  if (afl->stop_soon) { return 1; } // 用户按下 Ctrl+C 时，afl->stop_soon 会被设置为 1，此时 common_fuzz_stuff 函数会返回 1，通知 fuzz_one 函数终止 fuzzing 过程。

  if (fault == FSRV_RUN_TMOUT) {  // 如果目标程序执行超时，afl->subseq_tmouts
                                  // 计数器会增加，如果连续超时的次数超过了
                                  // TMOUT_LIMIT，common_fuzz_stuff 函数会返回
                                  // 1，通知 fuzz_one 函数终止 fuzzing
                                  // 过程。这是为了避免在 fuzzing
                                  // 过程中因为目标程序的性能问题而导致过多的无效测试用例执行，从而提高
                                  // fuzzing 的效率和效果。
  ...
  afl->queued_discovered += save_if_interesting(
      afl, out_buf, len,
      fault);  // 作用：保存有趣的测试用例，即在 fuzzing
               // 过程中根据目标程序的执行结果和覆盖率信息判断测试用例是否有趣，并将有趣的测试用例保存到队列中，以便在
               // fuzzing 过程中能够根据有趣的测试用例控制 fuzzing
               // 的行为和效率，促进 fuzzing 的效果提升
