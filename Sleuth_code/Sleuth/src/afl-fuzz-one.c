...
u8 fuzz_one_original(afl_state_t *afl) {  //模糊测试的主函数，负责对当前队列中的测试用例进行模糊测试。函数较长，返回值为0表示成功模糊测试，返回值为1表示跳过或退出。
  ...
  //若执行路径校验和超出当前主实例适用范围，则跳过确定性模糊测试
...
abandon_entry:
  ...
  if (!afl->stop_soon && !afl->queue_cur->cal_failed &&
    ...
    if (!afl->queue_cur->was_fuzzed) {
      ...
      update_pos_fuzzed(afl->queue_cur->position_hash);
      afl->reinit_table = 1;
