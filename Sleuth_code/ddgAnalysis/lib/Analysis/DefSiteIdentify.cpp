...
static cl::bits<DefSiteIdentify::DefSiteTypes> ClDefSitesToTrack(
    ...
    cl::values(clEnumValN(DefSiteIdentify::DefSiteTypes::Array,
               ...
               clEnumValN(DefSiteIdentify::DefSiteTypes::Pointer,
                          "fuzzalloc-def-pointer", "Track pointers (defs)")));
static cl::opt<bool>
...
bool DefSiteIdentify::runOnModule(Module &M) {
  ...
  if (ClDefSitesToTrack.isSet(DefSiteTypes::Pointer)) {
    status_stream() << "[" << M.getName() << "] Tracking pointer def sites\n";
  }
  ...
  for (const auto &[V, VI] : Vars) {
    ...
    if (ClDefSitesToTrack.isSet(DefSiteTypes::Array) && isa<ArrayType>(Ty)) {
      ToTrack.insert(V);
               ...
               isa<StructType>(Ty)) {
      ToTrack.insert(V);
    } else if (ClDefSitesToTrack.isSet(DefSiteTypes::Pointer) &&
               isa<PointerType>(Ty)){
      ToTrack.insert(V);
    }
  }
