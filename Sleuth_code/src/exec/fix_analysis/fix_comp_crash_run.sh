...
if grep -q $VULN $TABLE; then
	...
	SEEDSRC=$PROJECT'/comp_fuzz_'$1
	OUTPUT=$AFL_INFO'/fix_comp.txt'
	
	if [ -d $AFL_INFO'/fix_comp_crash_final' ]; then
		echo 'reseting crash dir...'
		rm -rf $AFL_INFO'/fix_comp_crash_final/'*
	else
		echo 'creating crash dir...'
		mkdir $AFL_INFO'/fix_comp_crash_final'
	fi
	...
	LOG_PATH='log_path='$AFL_INFO'/fix_comp_crash_final/asan.txt'
	DIRWATCH=$AFL_INFO'/fix_comp_crash_final'
	OPT='halt_on_error=0'
